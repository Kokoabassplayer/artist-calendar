...
`src/` contains the core extraction and storage logic: `image_to_structured.py` (Gemini extraction) and `local_db.py` (SQLite writes). `scripts/` provides runnable entry points such as the Flask ingest UI (`local_ingest_ui.py`) and a JSON ingest helper (`ingest_local.py`). `database/` holds `schema_local.sql` for the local database schema. `docs/` captures product notes and design context. Runtime outputs live under `output/` (uploads, SQLite DB) and are ignored by git.

## Build, Test, and Development Commands
- `python -m venv venv_artist && source venv_artist/bin/activate` (optional venv)
- `pip install -r requirements.txt` installs dependencies
- `python scripts/local_ingest_ui.py` runs the local ingestion UI
- `python -m playwright install chromium` installs browser support for complex URLs (optional)
- `python src/image_to_structured.py path/to/poster.jpg --output output/poster.json` extracts JSON from a poster
- `python scripts/ingest_local.py output/poster.json --db output/local.db` stores structured JSON in SQLite
- `python scripts/test_llm_api.py` verifies Gemini connectivity
...
## Testing Guidelines
There is no automated test suite yet. Use a quick manual smoke check: run the UI, upload a poster, and verify new rows appear in `output/local.db`.
