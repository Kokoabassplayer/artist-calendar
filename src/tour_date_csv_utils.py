...
def classify_images(input_csv, output_csv=None, output_folder=None):
    ...
    Args:
        ...
        output_folder (str, optional): Directory to save the output CSV. Defaults
            to the same directory as the input CSV.
    """
    # Load the CSV file
    ...
    if "Image URL" not in data.columns:
        raise ValueError(
            "The CSV file must have a column named 'Image URL' containing the image paths or URLs."
        )
    ...
    # If an output folder is specified, ensure it exists and construct the
    # full path
    if output_folder:
